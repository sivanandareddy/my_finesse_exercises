...
def scan_to_precision(DOF, target_precision, minmax="max", phi=0.0, precision=90.0, debug=None, extra_cmds=None):
    ...
    minmax - "max" or "min"scan_f_cmds(DOF, linlog, lower, upper, steps)
...
def scan_f(kat, DOF, linlog="lin", lower=10, upper=5000, steps=100, verbose=False):
    kat = kat.deepcopy()
    if DEBUG:
        cmd = scan_f_cmds(DOF, linlog, lower, upper, steps)
        print("The following commands are parsed.")
        print(cmd)
...
def scan_DOF(kat, DOF, xlimits=[-100, 100], steps=200, relative=False, extra_cmds=None): 
    ...
    if DEBUG:
        print("The following commands are parsed")
        print(scan_DOF_cmds(DOF, xlimits=xlimits, steps=steps, relative=relative))
...
def scan_optics(kat, _optics, _factors, target="phi", xlimits=[-100, 100], steps=200,relative=False, extra_cmds=None): 
    ...
    factors=make_list_copy(_factors)
    if DEBUG:
        print("The following commands will parsed")
        print(scan_optic_cmds(_optics, _factors, target=target, 
        xlimits=xlimits, steps=steps, relative=relative))
    kat.parse(scan_optic_cmds(_optics, _factors, target=target,
     xlimits=xlimits, steps=steps, relative=relative))
    if extra_cmds:
        if DEBUG:
            print("The following commands are parsed")
...
def optical_gain(DOF_sig, DOF_det, f=1, useDiff=True, deriv_h=1.0e-8):
    ...
    if DEBUG:
        print("The following commands will be parsed")
        print(_sigStr)
        print(_detStr)
...
def optimise_demod_phase(_kat, DOF, ports, minimise=False, debug=False):
    ...
    if DEBUG:
        print("The followng lines are parsed")
        print( diff_DOF(DOF, DOF.sigtype))
        print( scan_demod_phase_cmds(pd_detectors))
...
def ASC_demod_phase(kat, asc_dof, output, xaxis=[-1e-6, 1e-6, 50], err_tol = 1e-5,
    ...
    if DEBUG:
        print("The following lines will be parsed")
        print(pykat.ifo.scan_DOF_cmds(asc_dof,xlimits=[xaxis[0],xaxis[1]],steps=xaxis[2],relative=False))
        print('yaxis abs:deg')
    _kat.parse(pykat.ifo.scan_DOF_cmds(asc_dof,xlimits=[xaxis[0],xaxis[1]],steps=xaxis[2],relative=False))
    _kat.parse('yaxis abs:deg')

    if DEBUG:
        print("#kat code starts here ##############################")
        print(_kat)
        print("#kat code ends######################################")
    out = _kat.run()
    y = out[cmd_str[1]]
